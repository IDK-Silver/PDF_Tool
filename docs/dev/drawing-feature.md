...
interface PenSettings {
  color: string        // 預設: #000000 (黑色)
  strokeWidth: number  // 預設: 2pt, 範圍: 1-10pt
  opacity: number      // 固定: 1.0
}
```
...
interface HighlighterSettings {
  color: string        // 預設: #FFFF00 (黃色)
  strokeWidth: number  // 預設: 12pt, 範圍: 8-24pt
  opacity: number      // 固定: 0.4
}
```
...
interface EraserSettings {
  size: number  // 預設: 20px, 範圍: 10-50px
  mode: 'stroke' | 'point'  // 預設: stroke
}
```
