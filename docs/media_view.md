...
- 目標（本階段僅限「圖片」與「PDF」）：
  ...
  - 編輯區支援雙檢視模式，允許跨文件移動或複製頁面，並維持快取一致性與檔案安全。
- 範圍：
  - 前端路由/元件/狀態管理串接。
...
## 決策摘要（已定稿）
- PDF 引擎：`pdfium-render`；PDFium 動態庫隨 App 打包，執行期動態連結。
...
- 透明處理：PDF 透明區域以白底扁平化。
- 快取：App Cache 目錄、總上限 300MB、LRU、並行 3、預取 ±1；依 `mtime` 失效。
...
- PDF 引擎：採用 `pdfium-render`，PDFium 動態庫隨 App 預先打包，執行期動態連結（非下載時載入）。
...
Rust 端檔案規劃：
- `src-tauri/src/lib.rs:1`：註冊 `.invoke_handler(tauri::generate_handler![greet, analyze_media])`
- `src-tauri/src/media.rs`：
  ...
  - `fn detect_image(path) -> Result<(u32,u32), MediaError>`：用 `image::image_dimensions`
  - `fn pdf_page_count(path) -> Result<usize, MediaError>`：用 `lopdf` 或 `pdfium` 取得頁數
  - `fn pdf_page_sizes(path) -> Result<Vec<(f32,f32)>, MediaError>`：讀取 `MediaBox`（pt）
  - `fn pdf_render_page(path, pageIndex, scale|dpi, rotateDeg, format) -> Result<PageRender, MediaError>`：以 `pdfium-render` 光柵化為位圖（PNG/WebP），輸出到 app 暫存目錄並回傳路徑
  - `fn pdf_delete_pages(path, indices, outputPath, replace) -> Result<...>`：使用 `lopdf` 或 `pdfium` 操作頁樹，輸出新檔或覆寫
...
- 完成任一變更後後端發送 `document_changed` 事件（`path`, `mtime`, `pageCount`, `replaced`），前端刷新清單與渲染並失效舊快取。
...
- 量測：若具備 `pageSizesPt`，以頁尺寸與縮放倍率預估高度；無尺寸時以第一次渲染量測作為快取。
...
## 快取與渲染管線
- 鍵值：快取鍵包含檔案路徑、檔案修改時間（mtime）、頁索引、縮放倍率或目標寬度、旋轉角度、輸出格式。
- 儲存：渲染圖片寫入 App Cache 目錄（`app_cache_dir()`），檔名以快取鍵雜湊生成，前端以 `convertFileSrc` 顯示。
- 驅動：每次滾動與縮放僅排程可視區域與預取區域的渲染，使用最多 3 條並行渲染工作，超出部分排隊。
- 預取：預設預取下一頁與上一頁各 1 頁，滾動方向改變時即時調整。
- 回收：總圖片快取上限 300MB，採 LRU 淘汰策略；啟動時清理過期檔（mtime 不符）與超限部分。
- 取消：離開可視與預取範圍時取消尚未開始的渲染請求，已進行中的渲染允許完成並進入快取。

## 檔案安全與原子寫入
- 修改操作一律以新檔寫入至同目錄暫存檔，完成與 `fsync` 後以原子換名覆蓋或輸出新檔。
- 當 `replace` 啟用時於同目錄產生備份副本，操作成功後保留至工作階段結束。
- 變更完成後更新 mtime，前端依事件清空舊快取並以新快取鍵命中。
...
- 執行：刪除動作呼叫 `pdf_delete_pages`，預設產出新檔，完成後更新當前文件路徑與頁數，並清除相關頁面的渲染快取。
...
- PDFium 佈署：動態庫隨 App 打包於資源目錄，執行期動態連結（建議於 `src-tauri/tauri.conf.json:1` 內的 `bundle.resources` 指定各平台動態庫）。
- Capabilities：
  - 既有 `core:default` 可行；若有額外需求另行增列。
  - 檔案寫入策略：渲染圖片寫入 App Cache 目錄；PDF 寫入新檔（非破壞），除非 `replace: true`。
...
## TODO 狀態（M0 專注）
- [x] 決策：PDF 引擎採用 `pdfium-render`，PDFium 動態庫隨 App 打包、執行期動態連結。
...
- [x] 決策：快取路徑使用 App Cache、總上限 300MB、LRU、並行 3、預取 ±1。
- [x] 決策：後端事件 `document_changed`（`path`, `mtime`, `pageCount`, `replaced`）。
- [ ] 實作：`src-tauri/src/media.rs` 指令骨架（`analyze_media`/`pdf_info`/`pdf_render_page`）。
- [ ] 實作：`src-tauri/Cargo.toml:1` 新增 `pdfium-render` 相依（含平台條件/feature）。
- [ ] 實作：`src-tauri/tauri.conf.json:1` 設定 `bundle.resources` 引入 PDFium 動態庫。
- [ ] 實作：前端 `src/modules/media/{types.ts, service.ts, store.ts}` 骨架與路由串接。
- [ ] 實作：`src/components/MediaView/MediaView.vue:1` MVP（圖片直顯、PDF 單頁渲染清單、縮放框架）。
- [ ] 驗證：開發與打包在主要平台驗證（至少 macOS），確認動態連結成功。
- [ ] 驗證：快取清理、LRU 與 mtime 失效機制。
