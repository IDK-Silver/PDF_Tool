...
<template>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
    <div>
      ...
      <select v-model="s.imageFormat" class="w-full border rounded px-2 py-1">
    ...
    <div>
      ...
      <input class="w-full border rounded px-2 py-1" :value="s.imageDpi" @input="s.imageDpi = number($event, s.imageDpi)" />
      <p class="text-xs text-[hsl(var(--muted-foreground))] mt-1">72 = 1 倍，建議 150 或 300。</p>
    </div>
    <div>
      <label class="block mb-1">JPEG 品質（1–100）</label>
      <input class="w-full border rounded px-2 py-1" :disabled="s.imageFormat !== 'jpeg'" :value="s.imageQuality" @input="s.imageQuality = number($event, s.imageQuality)" />
