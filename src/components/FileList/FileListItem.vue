...
<template>
  ...
  >
    <button
      type="button"
      class="flex-1 h-full text-left px-[8px] border-0 bg-transparent"
      @click="onSelect"
    >
    ...
    <button v-if="removable" type="button" @click.stop="onRemove" title="從清單移除"
      class="mr-1 p-1 rounded hover:bg-[hsl(var(--accent))] transition-opacity opacity-0 group-hover:opacity-100 focus:opacity-100">
